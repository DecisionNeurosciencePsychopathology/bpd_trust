...
for j = 1:length(subdirs)
    ...
    if strcmp(os(1:end-2),'PCWIN')
        %datalocation = glob(['E:\data\bdp_trust\vba_out' filesep]);
...
%     pe_length = length(out.suffStat.PE);
%     P=zeros(num_of_subjects,pe_length); %CALCULATED PEs
%     M=zeros(num_of_subjects,pe_length); %MODEL PEs
%     N=zeros(num_of_subjects,pe_length); %value
