...
if hallquist_subjs
    ...
    rsfname=rsf.name;
    block_file = glob(['C:\kod\bpd_trust\subjects\', num2str(id), filesep, '*block_length.mat']);
...
if ~isempty(strfind(rsfname,'072916')) || ~isempty(strfind(rsfname,'08022016'))
    ...
    if isempty(block_file)
        ...
        load(block_file{:}); %This var needs to be block length
...
for index=1:num_of_subjects
    ...
    if not(str2double(id)==219956||str2double(id)==220017) %Remove later!
        ...
        %         figure(2);
        %         plot(b.notmissed_trials(total_trials-48:total_trials));
        %concatenating
        if blocknum>1
